...
from typing import Any, Optional, Union, TypeVar, Callable, NamedTuple
...
class OpCount(NamedTuple):
    flop_count: int
    mem_accessed: int

    def __add__(self, other: "OpCount") -> "OpCount":
        return OpCount(self.flop_count + other.flop_count, self.mem_accessed + other.mem_accessed)

    def __radd__(self, other: "OpCount") -> "OpCount":
        return self + other
...
def mm_flop(a_shape, b_shape, *args, out_shape=None, **kwargs) -> int:
    ...
    flop_count = m * n * 2 * k
    mem_accessed = m * k + k * n + m * n
    return OpCount(flop_count, mem_accessed)

@register_flop_formula(aten.addmm)
def addmm_flop(self_shape, a_shape, b_shape, out_shape=None, **kwargs) -> int:
    """Count flops for addmm."""
    return mm_flop(a_shape, b_shape) + OpCount(0, prod(self_shape) * 2)
...
def bmm_flop(a_shape, b_shape, out_shape=None, **kwargs) -> int:
    ...
    assert k == k2
    # NB(chilli): Should be 2 * k - 1 technically for FLOPs.
    flop_count = b * m * n * 2 * k
    mem_accessed = b * (m * k + k * n + m * n)
    return OpCount(flop_count, mem_accessed)
...
def baddbmm_flop(self_shape, a_shape, b_shape, out_shape=None, **kwargs) -> int:
    ...
    return bmm_flop(a_shape, b_shape) + OpCount(0, prod(self_shape) * 2)
...
def conv_flop_count(
    ...
    return OpCount(flop, prod(x_shape) + prod(w_shape) + prod(out_shape))
...
def sdpa_flop_count(query_shape, key_shape, value_shape):
    ...
    # softmax
    total_flops += OpCount(0, b * h * s_q * s_k * 5)
    return total_flops
